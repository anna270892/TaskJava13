...
public class Poster {
    MoviesPoster[] movies = new MoviesPoster[0]; //создаем массив с инициализацией ноль
    ...
    public void addNewMovie(MoviesPoster movieTitle) { //movieTitle параметр
        int length = movies.length + 1; //переменная текущая длинна массива + 1
        MoviesPoster[] tmp = new MoviesPoster[length]; //создаем массив с длинной из переменной length
        for (int i = 0; i < movies.length; i++) { //копируем элементы в новый массив
            tmp[i] = movies[i];
        }
    ...
    public MoviesPoster[] getLastAdd() {
        int moviesLength = movies.length; //текущую длинну массива сохраняем в moviesLength
